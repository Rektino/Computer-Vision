...
static void help(char **argv) {
  printf(
      ...
      "Usage: %s\n"
      "     -w=<board_width>         # the calibration board horizontal size "
      "in inner corners "
      "for chessboard and in squares or circles for others like ChArUco or "
      "circles grid\n"
      "     -h=<board_height>        # the calibration board verical size in "
      "inner corners "
      "for chessboard and in squares or circles for others like ChArUco or "
      "circles grid\n"
      "     [-pt=<pattern>]          # the type of pattern: chessboard, "
      "charuco, circles, acircles\n"
      ...
      "(1 by default)\n"
      "     [-ms=<markerSize>]       # marker size in some user-defined units "
      "(0.5 by default)\n"
      "     [-ad=<arucoDict>]        # Aruco dictionary name for ChArUco "
      "board. "
      ...
      "DICT_APRILTAG_36h11\n"
      "     [-adf=<dictFilename>]    # Custom aruco dictionary file for "
      "ChArUco board\n"
...
static bool runCalibration(vector<vector<Point2f>> imagePoints, Size imageSize,
  ...
  int offset =
      patternType != CHARUCOBOARD ? boardSize.width - 1 : boardSize.width - 2;
...
int main(int argc, char **argv) {
  ...
  cv::CommandLineParser parser(
      argc, argv,
      "{help "
      "||}{w||}{h||}{pt|chessboard|}{n|10|}{d|1000|}{s|1|}{ms|0.5|}{ad|DICT_"
      "4X4_50|}{adf|None|}{o|out_camera_data.yml|}"
      "{op||}{oe||}{zt||}{a||}{p||}{v||}{V||}{su||}"
      "{oo||}{ws|11|}{dt||}"
      "{fx||}{fy||}{cx||}{cy||}"
  ...
  float grid_width =
      squareSize *
      (pattern != CHARUCOBOARD ? (boardSize.width - 1) : (boardSize.width - 2));
  ...
  if (dictFilename == "None") {
    std::cout << "Using predefined dictionary with id: " << arucoDict
              << std::endl;
    dictionary = aruco::getPredefinedDictionary(arucoDict);
  } else {
    std::cout << "Using custom dictionary from file: " << dictFilename
              << std::endl;
  ...
  cv::aruco::CharucoBoard ch_board(boardSize, squareSize, markerSize,
                                   dictionary);
  ...
  for (i = 0;; i++) {
    ...
    case CHARUCOBOARD: {
      ...
      found = pointbuf.size() ==
              (size_t)(boardSize.width - 1) * (boardSize.height - 1);
      break;
    }
    default:
    ...
    if (found) {
      if (pattern != CHARUCOBOARD)
        ...
        drawChessboardCorners(view,
                              Size(boardSize.width - 1, boardSize.height - 1),
                              Mat(pointbuf), found);
    }
