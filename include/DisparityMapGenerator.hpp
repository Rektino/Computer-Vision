 ...
 * 1. Image acquisition 2. Calibrate cameras-> find intrisic matrices (might be
 * known or calculated independently)
 * 3. Stereo calibration -> finding the relative rotation and translation
 * between the two cameras. 4. Stereo rectification -> editing the images so
 * that corresponding points lie on the same horizontal lines.  5. Stereo
 * matching algorithm to compute the pixel-wise disparities between the images.
 */
...
public:
  DisparityMapGenerator(const std::string &path_img1,
